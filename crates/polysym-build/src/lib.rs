...
/// Cache manifest format version. Bump this when the polysym-build rendering
/// pipeline changes in a way that affects committed cache bytes — e.g. tweaks
/// to [`apply_padding`], [`apply_svg_postprocess`], the `pHYs` chunk, sfsym
/// invocation arguments, or the Lucide rasterization pipeline.
///
/// Bumping invalidates every consumer's `polysym-assets/` cache: their builds
...
/// matters when tier 1 fails, which never happens during a refresh.
fn spec_signature(spec: &SymbolSpec) -> String {
    format!("size={} weight={} padding={:.4}", spec.size, spec.weight, spec.padding)
}

fn manifest_path(assets_dir: &Path) -> PathBuf {
    assets_dir.join("manifest.txt")
}

/// Result of attempting to read the cache manifest.
enum ManifestStatus {
    /// File exists and matches the current format version. Contains a map of
    /// symbol name → its stored signature line.
    Ok(std::collections::HashMap<String, String>),
    /// No manifest file exists yet (cache predates manifest support, or
    /// `polysym-assets/` is empty).
    Missing,
    /// Manifest exists but its `format=N` line doesn't match
...
fn read_manifest(assets_dir: &Path) -> ManifestStatus {
    let Ok(content) = std::fs::read_to_string(manifest_path(assets_dir)) else {
    ...
    for line in content.lines() {
        ...
        if let Some(v) = line.strip_prefix("format=") {
            match v.parse::<u32>() {
                Ok(n) => format = Some(n),
                Err(_) => return ManifestStatus::Malformed(format!("invalid format= value: {v}")),
            }
            continue;
        }
        let Some((name, sig)) = line.split_once(' ') else {
            return ManifestStatus::Malformed(format!(
                "expected '<name> <key>=<value>...', got: {line}"
            ));
        };
        entries.insert(name.to_string(), sig.trim().to_string());
    }

    let Some(found) = format else {
        return ManifestStatus::Malformed("missing 'format=' header".into());
    };
    if found != CACHE_FORMAT_VERSION {
        return ManifestStatus::Outdated { found };
    }
    ManifestStatus::Ok(entries)
}

/// During a refresh, remove cache files in `assets_dir` whose symbol no longer
/// appears in `specs`. Keeps the committed cache in lockstep with `build.rs`,
/// so removing a symbol cleans up the orphans on the next refresh instead of
/// leaving them as silent cruft. The manifest itself is left alone — it gets
/// rewritten right after this.
fn prune_orphans(assets_dir: &Path, specs: &[SymbolSpec]) {
    let valid: HashSet<&str> = specs.iter().map(|s| s.name.as_str()).collect();
    ...
    for entry in entries.flatten() {
        ...
        let stem = file_name
            .strip_suffix(".light.png")
            .or_else(|| file_name.strip_suffix(".dark.png"))
            .or_else(|| file_name.strip_suffix(".svg"));
        ...
        if !valid.contains(stem) {
            match std::fs::remove_file(&path) {
                Ok(()) => println!("cargo::warning=polysym: pruned orphan cache file {file_name}"),
                Err(e) => println!(
                    "cargo::warning=polysym: failed to prune orphan {}: {e}",
                    path.display()
                ),
            }
        }
...
fn write_manifest(assets_dir: &Path, entries: &[(String, String)]) {
    let mut content = String::new();
    let _ = writeln!(content, "# polysym cache manifest - regenerated by POLYSYM_REFRESH=1");
    let _ = writeln!(content, "# DO NOT EDIT BY HAND");
    let _ = writeln!(content, "format={CACHE_FORMAT_VERSION}");
    let mut sorted: Vec<&(String, String)> = entries.iter().collect();
    sorted.sort_by(|a, b| a.0.cmp(&b.0));
    for (name, sig) in sorted {
        let _ = writeln!(content, "{name} {sig}");
    }
    let path = manifest_path(assets_dir);
    std::fs::write(&path, content)
        .unwrap_or_else(|e| panic!("polysym: failed to write manifest {}: {e}", path.display()));
}
...
pub fn generate_with_opts(specs: &[SymbolSpec]) {
    ...
    // Read the cache manifest once. Outdated/malformed manifests get a single
    // top-level warning so we don't spam one per symbol.
    ...
    if !refresh && sfsym_available {
        match &manifest_status {
            ManifestStatus::Outdated { found } => {
                println!(
                    "cargo::warning=polysym: cache manifest format is v{found}, polysym-build expects v{CACHE_FORMAT_VERSION} - run scripts/polysym-refresh.sh before commit",
                );
            }
            ManifestStatus::Malformed(msg) => {
                println!(
                    "cargo::warning=polysym: cache manifest is malformed ({msg}) - run scripts/polysym-refresh.sh before commit",
                );
            }
            ManifestStatus::Missing if assets_dir_populated => {
                println!(
                    "cargo::warning=polysym: cache has no manifest yet - run scripts/polysym-refresh.sh once to track configuration",
                );
            }
            _ => {}
        }
    }
    let manifest_entries = match &manifest_status {
        ManifestStatus::Ok(map) => Some(map),
        _ => None,
    };
    ...
    let mut new_manifest: Vec<(String, String)> = Vec::new();
    for spec in specs {
        let resolution = if sfsym_succeeded.contains(&spec.name) {
            ...
            println!(
                "cargo::warning=polysym: '{}' has no committed asset and no .lucide() fallback - \
                 omitted from SfIcons (will return None at runtime). \
                 Add .lucide(\"name\") to its SymbolSpec for cross-platform graceful degradation.",
                spec.name,
            );
    ...
    if refresh {
        prune_orphans(&assets_dir, specs);
        if !new_manifest.is_empty() {
            write_manifest(&assets_dir, &new_manifest);
        }
    }
...
/// Verify the committed cache for one symbol. Two checks:
///
/// 1. **Files exist.** All of `<name>.light.png`, `<name>.dark.png`,
///    `<name>.svg` must be present in `assets_dir`.
/// 2. **Spec hasn't drifted.** The manifest's stored signature for this
///    symbol must match the current [`spec_signature`].
///
/// Byte equality is intentionally not checked — sfsym output can vary in
...
fn check_committed_asset(
    ...
    // No usable manifest: top-level warning was already emitted; don't
    // duplicate it per symbol.
    let Some(manifest) = manifest else {
        return;
    };

    let current = spec_signature(spec);
    match manifest.get(&spec.name) {
        Some(stored) if stored == &current => {}
        Some(stored) => {
            println!(
                "cargo::warning=polysym: '{}' cache built with [{}] but spec is now [{}] - \
                 run scripts/polysym-refresh.sh before commit",
                spec.name, stored, current,
            );
        }
        None => {
            println!(
                "cargo::warning=polysym: '{}' is not listed in the cache manifest - \
                 run scripts/polysym-refresh.sh before commit",
                spec.name,
            );
        }
